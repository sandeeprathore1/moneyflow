...
def get_dashboard(db: Session, user_id: UUID, month: date | None = None) -> DashboardResponse:
    ...
    expenses = sum(
        ...
        if t.transaction_type in (TransactionType.EXPENSE, TransactionType.EMI)
    ) or Decimal("0")
...
def get_monthly_analytics(db: Session, user_id: UUID, month: date | None = None) -> MonthlyAnalyticsResponse:
    ...
    spent = sum_type((TransactionType.EXPENSE, TransactionType.EMI), start, end)
    income = sum_type((TransactionType.INCOME,), start, end)
    prev_spent = sum_type((TransactionType.EXPENSE, TransactionType.EMI), prev_start, prev_end)
